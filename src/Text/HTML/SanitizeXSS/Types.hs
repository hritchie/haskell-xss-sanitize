...
-- we want state monad so we know if we are IN an unsafe tag like script
-- because it is better to suppress and report the content of script tag

-- are we in an unsafe tag?
data XssState = XssState {
    -- not sure if we want to keep a tag stack here,
    -- perhaps sufficient if we know whether we are in a script tag
